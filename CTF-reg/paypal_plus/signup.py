...
MANUAL_OTP_FILE = os.environ.get("PPS_PAYPAL_MANUAL_OTP_FILE", "")
...
def fetch_persona(*, proxy: Optional[str] = None, timeout: int = 20) -> Persona:
    ...
    email = f"{_rand_alnum(17)}@gmail.com"
...
def _active_sms_api_url() -> str:
    if "PPS_SMS_API_URL" in os.environ:
        return (os.environ.get("PPS_SMS_API_URL") or "").strip()
    return (SMS_API_URL or "").strip()


def _sms_gateway_text(proxy: Optional[str] = None) -> str:
    sms_api_url = _active_sms_api_url()
    if not sms_api_url:
        return ""
    s = _make_session(proxy)
    try:
...
def _extract_sms_code_from_text(text: str, *, after_ts: float = 0.0) -> str:
    ...
    if not raw:
        return ""
    min_ts = float(after_ts or 0.0)
    # SMS vendors can be a few seconds out of sync with local click time.
    min_ts_with_skew = max(0.0, min_ts - 15.0) if min_ts else 0.0

    def _record_time(record: dict[str, Any]) -> float:
        for key in ("smsTime", "expiresTime", "time", "createdAt", "created_at", "receivedAt", "receiveTime", "timestamp"):
    ...
    def _code_from_value(value: Any, *, plain_digits: bool = False) -> str:
        if value in (None, ""):
            return ""
        ...
        if plain_digits:
            ...
            if 4 <= len(digits) <= 8:
                return digits
        patterns = [
            r"(?:code|verification|security|one[-\s]*time|passcode|pin|验证码|驗證碼)[^\d]{0,80}(\d{4,8})(?!\d)",
            r"(?<!\d)(\d{4,8})(?!\d)[^\n\r]{0,80}(?:code|verification|security|one[-\s]*time|passcode|pin|验证码|驗證碼)",
        ]
        for pat in patterns:
            m = re.search(pat, payload_text, re.I)
            if m:
                return m.group(1)
        return ""

    def _code_from_record(record: Any) -> str:
        ...
        for key in ("code", "otp", "pin"):
            ...
            if code:
                return code
        for key in ("sms", "text", "message", "content", "body"):
            code = _code_from_value(record.get(key))
            if code:
                return code
        app_name = str(record.get("appName") or record.get("app") or "").strip().lower()
        if app_name == "paypal":
            for key in ("sms", "text", "message", "content", "body"):
                payload_text = str(record.get(key) or "")
                m = re.search(r"PayPal[^\d]{0,32}(\d{6})(?!\d)", payload_text, re.I)
                if m:
                    return m.group(1)
        return ""

    def _code_from_records(records: list[Any]) -> str:
    ...
    # NexSMS json_latest:
    # {"code":0,"data":{"text":"...292633","code":"292633", ...}}
    if raw[:1] in "{[":
        try:
            data = json.loads(raw)
        except Exception:
        ...
        if isinstance(data, list):
            return _code_from_records(data)
        if isinstance(data, dict):
            payload = data.get("data")
            if isinstance(payload, dict):
                for list_key in ("list", "records", "rows", "items"):
                    records = payload.get(list_key)
                    if isinstance(records, list):
                        return _code_from_records(records)
                code = _code_from_record(payload)
                if code:
                    return code
            code = _code_from_record(data)
            if code:
                return code
        return ""
...
def wait_for_sms_otp(
    *,
    ...
    """
    if "PPS_PAYPAL_MANUAL_OTP_FILE" in os.environ:
        manual_path = (os.environ.get("PPS_PAYPAL_MANUAL_OTP_FILE") or "").strip()
    else:
        manual_path = (MANUAL_OTP_FILE or "").strip()
    if manual_path:
        ...
        try:
            ...
            logger.warning("manual otp file cleanup failed: %s", e)
        marker = f"PAYPAL_NEW_USER_OTP_REQUEST path={path}"
                ...
                if path.exists():
                    ...
                    if 4 <= len(digits) <= 8:
                        try:
                            ...
                            pass
                        logger.info("manual otp received len=%d", len(digits))
                        return digits
            except Exception as e:  # noqa: BLE001
                logger.warning("manual otp read error: %s", e)
            now = time.time()
            if now - last_log >= 30:
                last_log = now
                logger.info("waiting for manual PayPal OTP file: %s", path)
            time.sleep(1)
        raise TimeoutError(f"manual sms otp not received within {timeout}s (path={path})")

    sms_api_url = _active_sms_api_url()
    if not sms_api_url:
        raise RuntimeError("PPS_SMS_API_URL is empty and PPS_PAYPAL_MANUAL_OTP_FILE is not configured")
    ...
    while time.time() < deadline:
        ...
        code = _extract_sms_code_from_text(text, after_ts=after_ts)
        if code:
            return code
        time.sleep(poll_interval)
...
def _solve_recaptcha_v3_token_protocol(
    *,
    ...
    if proxy_fields:
        strategies.append({"type": "RecaptchaV3EnterpriseTask", **base_task, **proxy_fields})
    strategies.extend([
        {"type": "RecaptchaV3EnterpriseTaskProxyless", **base_task},
        {"type": "RecaptchaV3TaskProxyless", **base_task},
        {"type": "RecaptchaV3EnterpriseTask", **base_task},
    ])
    seen: set[str] = set()
    unique_strategies: list[dict[str, Any]] = []
    for task in strategies:
        key = json.dumps(task, sort_keys=True, separators=(",", ":"))
        if key not in seen:
            seen.add(key)
            unique_strategies.append(task)
    for idx, task in enumerate(unique_strategies, 1):
        logger.info("recaptcha captcha strategy %d/%d type=%s", idx, len(unique_strategies), task.get("type"))
        token, _solution = _poll_captcha_gateway(
            task=task,
            timeout=timeout,
            label=f"recaptcha[{task.get('type')}]",
