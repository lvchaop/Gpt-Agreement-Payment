...
OUT = PROTO / "goal_audit/h2_fresh_stack_only_control_audit.json"

DIRECT = PROTO / "direct_webshare_attempt/direct_webshare_attempt_ibtvqcnm-JP-1781240174000_1781240174.json"
COMBO = PROTO / "seq5_seq6_combo_probe/seq5_seq6_combo_probe_0a0e399e-661b-11f1-b302-62666cc2b93d_1781240263.json"
SEQ5_ACTIVITY = PROTO / "px561_compare/seq5_seq6_combo_probe_0a0e399e-661b-11f1-b302-62666cc2b93d_1781240263_seq5_full_activity_diff.json"
SEQ5_FIELD = PROTO / "px561_compare/seq5_seq6_combo_probe_0a0e399e-661b-11f1-b302-62666cc2b93d_1781240263_seq5_full_field_diff.json"
SEQ6_ACTIVITY = PROTO / "px561_compare/seq5_seq6_combo_probe_0a0e399e-661b-11f1-b302-62666cc2b93d_1781240263_seq6_full_activity_diff.json"
...
def main() -> int:
    ...
    audit = {
        "purpose": "Single-variable control for PX561 stack material: keep accepted template tail, inner uuid, and non-PX decoded activities, but inject fresh stack W0shQR0nJHc= from the same-session first-failure PX561 probe.",
        "inputs": {
            "direct": str(DIRECT),
            "combo": str(COMBO),
        ...
        "checks": {
            ...
            "usesFreshProbeStackOnly": meta.get("stackSource") == "fresh-probe"
            and bool(meta.get("stackProbe"))
            and meta.get("aeaxSource") == "template"
            and meta.get("bztSource") == "template"
            and meta.get("tailSource") == "template"
            and meta.get("innerUuidSource") == "template"
            and meta.get("nonPxActivitySource") == "template",
            "onlyStackDiffVsS00": seq5_field.get("diffCountVsS00Success") == 1
            and (seq5_field.get("diffsByClass") or {}) == {"other": ["W0shQR0nJHc="]},
        ...
        "conclusion": (
            "Injecting the same-session first-failure fresh PX561 stack W0shQR0nJHc= as the only decoded seq5 PX561 difference, while preserving accepted template tail/inner uuid/non-PX activities and strongest h2/asset/timing lineage, still returns oIIoIooo|-1. "
            "Therefore stack freshness alone does not close the pure-protocol HUMAN acceptance gap."
        ),
    }
