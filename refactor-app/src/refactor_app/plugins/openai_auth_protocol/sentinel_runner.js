...
const SESSION_OBSERVER_EXPORT_REPLACEMENT =
  "return o?r?.[n(63)]?ce({so:o,c:r[n(63)]},t):o:null},t.token=ye,t.__codexBindProof=D,t.__codexTurnstileToken=_n,t.__codexSessionObserverToken=async(challenge,flow)=>{se(flow,challenge);await new Promise(r=>setTimeout(r,600));return await t.sessionObserverToken(flow)},t}({});";
...
function exposeSessionObserverToken(sdkCode) {
  ...
  return sdkCode.replace(
    SESSION_OBSERVER_EXPORT_PATTERN,
    SESSION_OBSERVER_EXPORT_REPLACEMENT
  );
}

function sessionObserverValue(rawValue) {
  if (!rawValue) return "";
  const parsed = typeof rawValue === "string" ? parseJson(rawValue, "Session Observer Token") : rawValue;
  if (parsed && typeof parsed === "object") return String(parsed.so || "");
  return "";
}
...
async function main(argv = process.argv.slice(2), writeOutput = true) {
  ...
  let sdkCode = exposeSessionObserverToken(fs.readFileSync(sdkPath, "utf8"));
  if (debugDx) {
  ...
  if (cachedChallenge?.turnstile?.dx) {
    if (!cachedProof) {
      throw new Error("SDK Turnstile challenge 缺少 requirements proof");
    }
    if (
      typeof context.SentinelSDK.__codexBindProof !== "function" ||
      typeof context.SentinelSDK.__codexTurnstileToken !== "function"
    ) {
      throw new Error("SDK 未暴露 Turnstile Token 生成器");
    }
    context.SentinelSDK.__codexBindProof(cachedChallenge, cachedProof);
    const turnstileToken = await context.SentinelSDK.__codexTurnstileToken(
      cachedChallenge,
      cachedChallenge.turnstile.dx
    );
    if (!turnstileToken) {
      throw new Error("Sentinel challenge 要求 Turnstile，但 SDK 返回为空");
    }
    token.t = String(turnstileToken);
    tokenText = JSON.stringify(token);
  }
  if (cachedChallenge?.so?.required) {
    if (typeof context.SentinelSDK.__codexSessionObserverToken !== "function") {
      throw new Error("SDK 未暴露 Session Observer Token 生成器");
    }
    const rawSessionObserverToken = await context.SentinelSDK.__codexSessionObserverToken(
      cachedChallenge,
      flow
    );
    const so = sessionObserverValue(rawSessionObserverToken);
    if (!so) {
      throw new Error("Sentinel challenge 要求 Session Observer，但 SDK 返回为空");
    }
    token.so = so;
    tokenText = JSON.stringify(token);
  }
