...
@dataclass(frozen=True)
class PreparedChatGPTInviteRequest:
    path: str
...
class OpenAIChatGPTClient:
    ...
    def invite_member(
        ...
        *,
        ...
        return validate_invite_member_payload(email=email, payload=payload)
    ...
    def invite_members(
        ...
        *,
        ...
        prepared = prepare_invite_members_request(
            access_token=access_token,
            team_id=team_id,
            emails=emails,
            cookie_header=cookie_header,
        ...
        return parse_chatgpt_response_payload(response)
    ...
    def revoke_account_invite(
        ...
        *,
        ...
        if not normalized_email or "@" not in normalized_email:
            raise OpenAIChatGPTClientError("a valid invite email is required")
        response = self._chatgpt_delete(
            ...
            json={"email_address": normalized_email},
            proxy_url=proxy_url,
        )
        return _response_payload(response)
    ...
    def _chatgpt_get(
        ...
        *,
        ...
        with curl_requests.Session(impersonate=BROWSER_IMPERSONATE, proxies=proxies) as session:
            return session.get(
                f"{self._config.chatgpt_base_url}{path}",
                headers=headers,
                timeout=self._config.timeout_s,
    ...
    def _chatgpt_delete(
        ...
        *,
        ...
        try:
            if proxy_url == "" and self._chatgpt_http_client_injected:
                return self._chatgpt_client.request(
                    "DELETE",
                    path,
                    headers=headers,
                    json=json,
...
def prepare_invite_members_request(
    *,
    access_token: str,
    team_id: str,
    emails: list[str],
    ...
    return PreparedChatGPTInviteRequest(
        path=f"/backend-api/accounts/{team_id}/invites",
        headers=_team_headers(
            access_token=access_token,
            team_id=team_id,
            referer="https://chatgpt.com/admin",
...
def validate_invite_member_payload(*, email: str, payload: dict) -> dict:
...
def _account_change_email_headers(
    *,
    ...
    return _wham_headers(
        access_token=access_token,
        cookie_header=(
            _minimal_web_session_cookie_header(cookie_header) if cookie_header else ""
        ),
        referer="https://chatgpt.com/",
    )
