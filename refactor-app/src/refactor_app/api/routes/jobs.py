...
def list_jobs(
    ...
    return page_payload(
        items=[{**_job_dict(job), "progress": progress.get(job.id, _empty_progress())} for job in jobs],
        page=page,
        page_size=page_size,
        total=total,
...
def get_run_events(
    ...
    stmt, normalized_sort = _apply_sort(
        stmt,
        sort,
        {"ts": JobEventModel.ts, "level": JobEventModel.level, "event_type": JobEventModel.event_type},
        default="ts",
    )
    total = total_for(session, stmt)
...
_SENSITIVE_KEY_RE = re.compile(
    ...
    r"authorization|admin_key|custom_auth_header_value|password|csrf_token|code_verifier|otp_code)($|_)",
    re.IGNORECASE,
)
...
def _redact_value(value: Any, *, key: str = "") -> Any:
    ...
    if isinstance(value, str):
        if key in {"request_url", "response_url", "location"}:
