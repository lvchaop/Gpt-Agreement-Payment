...
CREATE TABLE user_account_auth (
  ...
  personal_chatgpt_account_id TEXT NOT NULL DEFAULT '',
  personal_chatgpt_account_discovered_at TIMESTAMPTZ,
...
CREATE TABLE workspace_join_batch_items (
  ...
  codex_credential_id TEXT REFERENCES codex_oauth_credentials(id) ON DELETE RESTRICT,
  item_status TEXT NOT NULL CHECK (
    item_status IN ('pending', 'joined', 'token_generated', 'pushed', 'failed', 'skipped')
  ),
  batch_binding_status TEXT NOT NULL DEFAULT 'active' CHECK (batch_binding_status IN ('active', 'released')),
...
CREATE INDEX idx_batch_items_codex_credential_id ON workspace_join_batch_items(codex_credential_id);
...
CREATE UNIQUE INDEX uq_batch_items_one_active_credential
