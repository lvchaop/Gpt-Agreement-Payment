...
async function load() {
  ...
  try {
    ...
    [jobs.value, spaces.value, memberships.value, credentials.value, proxies.value, mailLeases.value] =
      result;
    store.touchRefresh();
  } catch (err) {
...
<template>
  <PageHeader title="总览" description="看 Space 主流程是否卡住、失败、缺代理、缺 token。">
  ...
  <div class="grid-4">
    ...
    <MetricCard label="有效空间" :value="spaces.filter((w) => w.space_status === 'active').length" tone="success" />
