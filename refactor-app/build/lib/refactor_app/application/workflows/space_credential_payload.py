...
def build_space_credential_payload(
    *,
    ...
    if credential.auth_mode == "codex_oauth":
        return (
            ...
            space.credential_type,
        )

    if credential.auth_mode != "backend_access_token":
        raise SpaceCredentialPayloadError(
            f"unsupported_credential_auth_mode:{credential.auth_mode}"
        )
    if space.credential_type == "personal_account":
        raise SpaceCredentialPayloadError("personal_credential_requires_codex_oauth")
